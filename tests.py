...
class TestBooksCollector:
    ...
    def test_lack_of_rating_of_a_book_that_is_not_in_the_list(self):
        collector = BooksCollector()
    ...
    def test_rating_less_one(self, collector):
        ...
        collector.set_book_rating("Любая книга", 5)
        assert collector.get_books_rating("Любая книга")== 5, "Доступен рейтинг < 1"
    def test_rating_more_ten(self, collector,White_Bim_Black_ear):
        collector = BooksCollector()
        collector.set_book_rating(list(collector.books_rating())[0], 11)
        assert list(collector.books_rating.values())[0] == 1, "Доступен рейтинг > 10"
    ...
    def test_deleting_a_book_from_favorites(self, collector, White_Bim_Black_ear):
        collector = BooksCollector()
        collector.add_book_in_favorites(list(collector.get_books_rating.keys())[0])
    ...
    def test_getting_a_book_with_the_right_rating(self, collector, White_Bim_Black_ear, Kolobok, Ni_noy):
        ...
        assert len(collector.get_books_with_specific_rating(1)) == 1, "Книги с не правильным рейтингом"
    def test_set_book_rating(self, White_Bim_Black_ear):
        collector = BooksCollector()
